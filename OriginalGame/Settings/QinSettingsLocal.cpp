...
#include "OriginalGame/Player/QinLocalPlayer.h"
#include "PlayerMappableInputConfig.h"
#include "EnhancedInputSubsystems.h"
#include "ICommonUIModule.h"
#include "CommonUISettings.h"
#include "Widgets/Layout/SSafeZone.h"
#include "Performance/QinPerformanceSettings.h"
#include "DeviceProfiles/DeviceProfileManager.h"
#include "DeviceProfiles/DeviceProfile.h"
#include "HAL/PlatformFramePacer.h"
#include "Development/QinPlatformEmulationSettings.h"
#include "Performance/QinPerformanceStatTypes.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(QinSettingsLocal)
...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("Qin.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("Qin.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("Qin.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("Qin.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("Qin.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("Qin.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("Qin.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("Qin.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("Qin.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("Qin.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FQinScalabilitySnapshot::FQinScalabilitySnapshot()
{
...
namespace QinSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FQinScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
	...
	{
		const UQinPlatformSpecificRenderingSettings* PlatformSettings = UQinPlatformSpecificRenderingSettings::Get();
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = UQinSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : UQinSettingsLocal::GetDefaultMobileFrameRate();
	}
...
UQinSettingsLocal::UQinSettingsLocal()
{
...
void UQinSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
...
void UQinSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const UQinPlatformSpecificRenderingSettings* PlatformSettings = UQinPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EQinFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}

	DesiredUserChosenDeviceProfileSuffix = UserChosenDeviceProfileSuffix;
	QinSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void UQinSettingsLocal::ResetToCurrentSettings()
{
...
void UQinSettingsLocal::BeginDestroy()
{
...
UQinSettingsLocal* UQinSettingsLocal::Get()
{
	return GEngine ? CastChecked<UQinSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UQinSettingsLocal::ConfirmVideoMode()
{
...
float UQinSettingsLocal::GetEffectiveFrameRateLimit()
{
	const UQinPlatformSpecificRenderingSettings* PlatformSettings = UQinPlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == EQinFramePacingMode::DesktopStyle)
	{
...
int32 UQinSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return QinSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UQinSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FQinScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UQinSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UQinSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UQinSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UQinSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void UQinSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UQinSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
EQinStatDisplayMode UQinSettingsLocal::GetPerfStatDisplayState(EQinDisplayablePerformanceStat Stat) const
{
	if (const EQinStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return EQinStatDisplayMode::Hidden;
	}
}

void UQinSettingsLocal::SetPerfStatDisplayState(EQinDisplayablePerformanceStat Stat, EQinStatDisplayMode DisplayMode)
{
	if (DisplayMode == EQinStatDisplayMode::Hidden)
	{
...
float UQinSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void UQinSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UQinSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float UQinSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void UQinSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UQinSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UQinSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UQinSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void UQinSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UQinSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UQinSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void UQinSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 UQinSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UQinSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UQinSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UQinSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return QinSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UQinSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return QinSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UQinSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UQinSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UQinPlatformSpecificRenderingSettings* PlatformSettings = UQinPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == EQinFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return QinSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void UQinSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const UQinPlatformSpecificRenderingSettings* PlatformSettings = UQinPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EQinFramePacingMode::MobileStyle)
	{
...
void UQinSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UQinSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UQinPlatformSpecificRenderingSettings* PlatformSettings = UQinPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EQinFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = QinSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void UQinSettingsLocal::ClampMobileQuality()
{
	const UQinPlatformSpecificRenderingSettings* PlatformSettings = UQinPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EQinFramePacingMode::MobileStyle)
	{
...
void UQinSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = QinSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void UQinSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = QinSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = QinSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString UQinSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UQinSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

bool UQinSettingsLocal::CanRunAutoBenchmark() const
{
	const UQinPlatformSpecificRenderingSettings* PlatformSettings = UQinPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UQinSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UQinSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void UQinSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

void UQinSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void UQinSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 UQinSettingsLocal::GetOverallScalabilityLevel() const
{
...
void UQinSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == EQinFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = QinSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void UQinSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName UQinSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void UQinSettingsLocal::RegisterInputConfig(ECommonInputType Type, const UPlayerMappableInputConfig* NewConfig, const bool bIsActive)
{
	if (NewConfig)
	{
...
int32 UQinSettingsLocal::UnregisterInputConfig(const UPlayerMappableInputConfig* ConfigToRemove)
{
	if (ConfigToRemove)
	{
...
const UPlayerMappableInputConfig* UQinSettingsLocal::GetInputConfigByName(FName ConfigName) const
{
...
void UQinSettingsLocal::GetRegisteredInputConfigsOfType(ECommonInputType Type, TArray<FLoadedMappableConfigPair>& OutArray) const
{
	OutArray.Empty();
...
void UQinSettingsLocal::GetAllMappingNamesFromKey(const FKey InKey, TArray<FName>& OutActionNames)
{
...
void UQinSettingsLocal::AddOrUpdateCustomKeyboardBindings(const FName MappingName, const FKey NewKey, UQinLocalPlayer* LocalPlayer)
{
...
void UQinSettingsLocal::ResetKeybindingToDefault(const FName MappingName, UQinLocalPlayer* LocalPlayer)
{
	if (UEnhancedInputLocalPlayerSubsystem* Subsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(LocalPlayer))
	{
...
void UQinSettingsLocal::ResetKeybindingsToDefault(UQinLocalPlayer* LocalPlayer)
{
	if (UEnhancedInputLocalPlayerSubsystem* Subsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(LocalPlayer))
	{
...
void UQinSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void UQinSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const UQinPlatformSpecificRenderingSettings* PlatformSettings = UQinPlatformSpecificRenderingSettings::Get();
	const TArray<FQinQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FQinQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const UQinPlatformEmulationSettings* Settings = GetDefault<UQinPlatformEmulationSettings>();
	...
	{
	case EQinFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case EQinFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case EQinFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void UQinSettingsLocal::UpdateConsoleFramePacing()
{
...
void UQinSettingsLocal::UpdateDesktopFramePacing()
{
...
void UQinSettingsLocal::UpdateMobileFramePacing()
{
...
void UQinSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
