...
UENUM(BlueprintType)
enum class EJoyAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
/**
 * UJoySettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
...
public:
	DECLARE_EVENT_OneParam(UJoySettingsShared, FOnSettingChangedEvent, UJoySettingsShared* Settings);
...
private:
	UPROPERTY()
	EJoyAllowBackgroundAudioSetting AllowAudioInBackground = EJoyAllowBackgroundAudioSetting::Off;
...
private:
	...
	UPROPERTY(Transient)
	TObjectPtr<UJoyLocalPlayer> OwningPlayer = nullptr;
};
