...
#include "JoySettingsLocal.generated.h"

enum class ECommonInputType : uint8;
enum class EJoyDisplayablePerformanceStat : uint8;
enum class EJoyStatDisplayMode : uint8;
...
USTRUCT()
struct FJoyScalabilitySnapshot
{
	GENERATED_BODY()

	FJoyScalabilitySnapshot();
 ...
 */
UCLASS()
class UJoySettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
private:
	...
	UPROPERTY(Config)
	TMap<EJoyDisplayablePerformanceStat, EJoyStatDisplayMode> DisplayStatList;
...
private:
	...
	FJoyScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
public:
	...
	DECLARE_EVENT_OneParam(UJoySettingsLocal, FInputConfigDelegate, const FLoadedMappableConfigPair& /*Config*/);
	 ...
	 * @param LocalPlayer   local player to reset the keybinding on
	 */
	void ResetKeybindingToDefault(const FName MappingName, UJoyLocalPlayer* LocalPlayer);
