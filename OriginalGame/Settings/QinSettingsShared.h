...
#include "QinSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EQinAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
UENUM(BlueprintType)
enum class EQinGamepadSensitivity : uint8
{
...
/**
 * UQinSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 *               maybe change Localsettings to QinPlatformSettings, or DeviceSettings?  Make this one UserSettings?  TBD
 *               
...
class UQinSettingsShared : public USaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UQinSettingsShared, FOnSettingChangedEvent, UQinSettingsShared* Settings);
...
public:
	...
	void Initialize(UQinLocalPlayer* LocalPlayer);

	bool IsDirty() const { return bIsDirty; }
	void ClearDirtyFlag() { bIsDirty = false; }

	void SaveSettings();
	static UQinSettingsShared* LoadOrCreateSettings(const UQinLocalPlayer* LocalPlayer);
...
public:
	UFUNCTION()
	EQinAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EQinAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EQinAllowBackgroundAudioSetting AllowAudioInBackground = EQinAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EQinGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EQinGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EQinGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EQinGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	EQinGamepadSensitivity GamepadLookSensitivityPreset = EQinGamepadSensitivity::Normal;
	UPROPERTY()
	EQinGamepadSensitivity GamepadTargetingSensitivityPreset = EQinGamepadSensitivity::Normal;
...
private:
	...
	UPROPERTY(Transient)
	TObjectPtr<UQinLocalPlayer> OwningPlayer = nullptr;
};
