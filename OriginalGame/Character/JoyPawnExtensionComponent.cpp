...
#include "AbilitySystem/JoyAbilitySystemComponent.h"
...
const FName UJoyPawnExtensionComponent::NAME_ActorFeatureName("PawnExtension");

UJoyPawnExtensionComponent::UJoyPawnExtensionComponent(const FObjectInitializer& ObjectInitializer)
...
void UJoyPawnExtensionComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(UJoyPawnExtensionComponent, PawnData);
}

void UJoyPawnExtensionComponent::OnRegister()
{
	Super::OnRegister();
...
void UJoyPawnExtensionComponent::BeginPlay()
{
	Super::BeginPlay();
...
void UJoyPawnExtensionComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void UJoyPawnExtensionComponent::SetPawnData(const UJoyPawnData* InPawnData)
{
	check(InPawnData);
...
void UJoyPawnExtensionComponent::OnRep_PawnData()
{
	CheckDefaultInitialization();
}

void UJoyPawnExtensionComponent::InitializeAbilitySystem(UJoyAbilitySystemComponent* InASC, AActor* InOwnerActor)
{
	check(InASC);
	check(InOwnerActor);
	...
	{
		UE_LOG(LogJoy, Log, TEXT("Existing avatar (authority=%d)"), ExistingAvatar->HasAuthority() ? 1 : 0);

		// There is already a pawn acting as the ASC's avatar, so we need to kick it out
		// This can happen on clients if they're lagged: their new pawn is spawned + possessed before the dead one is removed
		ensure(!ExistingAvatar->HasAuthority());

		if (UJoyPawnExtensionComponent* OtherExtensionComponent = FindPawnExtensionComponent(ExistingAvatar))
		{
...
void UJoyPawnExtensionComponent::UninitializeAbilitySystem()
{
	if (!AbilitySystemComponent)
	{
		return;
	...
	if (AbilitySystemComponent->GetAvatarActor() == GetOwner())
	{
		...
		AbilityTypesToIgnore.AddTag(JoyGameplayTags::Ability_Behavior_SurvivesDeath);
...
void UJoyPawnExtensionComponent::HandleControllerChanged()
{
...
void UJoyPawnExtensionComponent::HandlePlayerStateReplicated()
{
	CheckDefaultInitialization();
}

void UJoyPawnExtensionComponent::SetupPlayerInputComponent()
{
	CheckDefaultInitialization();
}

void UJoyPawnExtensionComponent::CheckDefaultInitialization()
{
...
bool UJoyPawnExtensionComponent::CanChangeInitState(UGameFrameworkComponentManager* Manager, FGameplayTag CurrentState, FGameplayTag DesiredState) const
{
	...
	if (CurrentState == JoyGameplayTags::InitState_Spawned && DesiredState == JoyGameplayTags::InitState_DataAvailable)
	{
...
void UJoyPawnExtensionComponent::OnActorInitStateChanged(const FActorInitStateChangedParams& Params)
{
	// If another feature is now in DataAvailable, see if we should transition to DataInitialized
	if (Params.FeatureName != NAME_ActorFeatureName)
	{
		if (Params.FeatureState == JoyGameplayTags::InitState_DataAvailable)
		{
			CheckDefaultInitialization();
		}
	}
...
void UJoyPawnExtensionComponent::OnAbilitySystemUninitialized_Register(FSimpleMulticastDelegate::FDelegate Delegate)
{
