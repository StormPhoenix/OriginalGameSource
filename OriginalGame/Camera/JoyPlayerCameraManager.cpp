...
DECLARE_CYCLE_STAT(TEXT("Camera ProcessViewRotation"),
                   STAT_Camera_ProcessViewRotation, STATGROUP_Game);
...
void FVirtualCamera::SetArmCenterRotation(
    const FRotator &NewArmCenterRotation) {
  ArmCenterRotation = NewArmCenterRotation;
}

void AJoyPlayerCameraManager::BlendViewInfo(FMinimalViewInfo &A,
                                            FMinimalViewInfo &B, float T) {
...
AJoyPlayerCameraManager::AJoyPlayerCameraManager(
...
void AJoyPlayerCameraManager::UpdateViewTarget(FTViewTarget &OutVT,
                                               float DeltaTime) {
...
void AJoyPlayerCameraManager::UpdateCameraControllers(float DeltaTime) {
  for (auto ItViewTarget = MultiViewTargetCameraManager.ViewTargetCameraInfos
                               .CreateConstIterator();
...
void AJoyPlayerCameraManager::UpdateViewTargetPose() {
  const auto *CharacterControlManager =
      UJoyCharacterControlManageSubsystem::Get(GetWorld());
  for (TMap<TWeakObjectPtr<AActor>,
            FViewTargetCameraInfo>::TIterator ItViewTarget =
    ...
    if (CharacterControlManager) {
      if (NewViewTarget ==
          CharacterControlManager->GetCurrentControlCharacter()) {
        bViewTargetFound = true;
      }
    }

    if (bViewTargetFound || NewViewTarget == ViewTarget.Target ||
        NewViewTarget == PendingViewTarget.Target) {
      // 此处做一些对 DesiredCamera 数据的重置操作
      CameraInfo.DesiredCamera.ArmCenterRotation =
          CameraInfo.CurrentCamera.ArmCenterRotation;
    }
  }
}

bool AJoyPlayerCameraManager::NeedUpdateViewTarget(
    AActor *InViewTarget, const FViewTargetCameraInfo &CameraInfo) const {
  /** ******************************************************************************** **
  ...
  if (InViewTarget != nullptr) {
    if (ViewTarget.Target == InViewTarget ||
        PendingViewTarget.Target == InViewTarget) {
      return true;
    }
  ...
  for (TMap<TWeakObjectPtr<AActor>,
    ...
    FRotator LocalRotator =
        GravityManager != nullptr
            ? GravityManager->WorldRotatorToLocal(WorldRotator)
            : WorldRotator;
    LocalRotator.Pitch =
        FMath::Clamp(LocalRotator.Pitch, MinArmPitch, MaxArmPitch);
    WorldRotator = GravityManager != nullptr
                       ? GravityManager->LocalRotatorToWorld(LocalRotator)
                       : LocalRotator;
    ...
    const FRotator CameraSpace =
        FRotator(ViewRotator.Pitch, ViewRotator.Yaw, ViewRotator.Roll);
    ...
    CameraInfo.CurrentCamera.SetArmCenterOffset(
        CameraInfo.CurrentCamera.LocalArmCenterOffset.X * ForwardVec +
        CameraInfo.CurrentCamera.LocalArmCenterOffset.Y * RightVec +
        CameraInfo.CurrentCamera.LocalArmCenterOffset.Z * UpVec);

    CameraInfo.CurrentCamera.SetArmCenterOffset(
        CameraInfo.CurrentCamera.ArmCenterOffset +
        CameraInfo.CurrentCamera.WorldArmOffsetAdditional);
  }
}

void AJoyPlayerCameraManager::SetRotationInternal(const AActor *InViewTarget,
                                                  FRotator Rotator) {
  if (InViewTarget == nullptr) {
    return;
  }
...
void AJoyPlayerCameraManager::DoUpdateCamera(float InDeltaTime) {
  ...
  if (bEnableColorScaleInterp) {
    const float BlendPct =
        FMath::Clamp((GetWorld()->TimeSeconds - ColorScaleInterpStartTime) /
    ...
    if (BlendTimeToGo > 0) {
      ...
      if (!BlendViewCurve.Get()) {
        ...
        case VTBlend_Cubic:
          ...
          break;
        case VTBlend_EaseIn:
          BlendPct = FMath::Lerp(0.f, 1.f,
                                 FMath::Pow(DurationPct, BlendParams.BlendExp));
          break;
        case VTBlend_EaseOut:
          BlendPct = FMath::Lerp(
              0.f, 1.f, FMath::Pow(DurationPct, 1.f / BlendParams.BlendExp));
          break;
        case VTBlend_EaseInOut:
          BlendPct = FMath::InterpEaseInOut(0.f, 1.f, DurationPct,
                                            BlendParams.BlendExp);
          break;
        ...
        BlendPct =
            FMath::Clamp(BlendViewCurve->GetFloatValue(DurationPct), 0.f, 1.f);
      }
      ...
      OnViewTargetBlendComplete.Broadcast(ViewTarget.Target,
  ...
  if (bEnableFading) {
    if (bAutoAnimateFade) {
      FadeTimeRemaining = FMath::Max(
          FadeTimeRemaining - DeltaTimeThisFrame_IgnoreTimeDilation, 0.0f);
      if (FadeTime > 0.0f) {
        FadeAmount = FadeAlpha.X + ((1.f - FadeTimeRemaining / FadeTime) *
                                    (FadeAlpha.Y - FadeAlpha.X));
      }
  ...
  if (AllowPhotographyMode()) {
    ...
    bGameCameraCutThisFrame =
        bGameCameraCutThisFrame || bPhotographyCausedCameraCut;
  }
...
void AJoyPlayerCameraManager::AddNewViewTarget(AActor *NewViewTarget) {
  ...
  MultiViewTargetCameraManager.AddViewTarget(this, NewViewTarget,
                                             VirtualCamera);
}
...
void AJoyPlayerCameraManager::InitializeFor(APlayerController *PC) {
  ...
  for (TPair<TWeakObjectPtr<AActor>, FViewTargetCameraInfo> &ModifierContainer :
    ...
    CameraInfo.CameraModifierController =
        NewObject<UJoyCameraModifierController>(this);
    check(CameraInfo.CameraModifierController);
    CameraInfo.CameraModifierController->InitializeFor(this);
    CameraInfo.CameraModifierController->ModifiedViewTarget =
        NewViewTarget.Get();
  }

  Super::InitializeFor(PC);
  FGameDelegates::Get().GetViewTargetChangedDelegate().AddUObject(
      this, &ThisClass::OnViewTargetChanged);
}
...
float AJoyPlayerCameraManager::GetBaseCameraFov() const { return BasicFov; }

FRotator AJoyPlayerCameraManager::GetViewTargetViewRotation(
...
FVector AJoyPlayerCameraManager::GetNegativeGravityNormal() const {
  const auto *GravityManager = UJoyGravityManageSubsystem::Get(GetWorld());
  return GravityManager != nullptr ? GravityManager->GetGravitySpaceZ()
                                   : FVector(0., 0., 1.f);
}

FVector
AJoyPlayerCameraManager::GetCharacterHeadLocation(const AActor *Target) const {
  if (Target == nullptr) {
  ...
  if (const APawn *TargetPawn = Cast<APawn>(TargetActor)) {
    ...
    if (const ACharacter *TargetCharacter = Cast<ACharacter>(TargetPawn)) {
      ...
      const UCapsuleComponent *CapsuleComp =
          TargetCharacter->GetCapsuleComponent();
      check(CapsuleComp);

      const UCapsuleComponent *CapsuleCompCDO =
          TargetCharacterCDO->GetCapsuleComponent();
      check(CapsuleCompCDO);

      const float DefaultHalfHeight =
          CapsuleCompCDO->GetUnscaledCapsuleHalfHeight();
      const float ActualHalfHeight =
          CapsuleComp->GetUnscaledCapsuleHalfHeight();
      const float HeightAdjustment = (DefaultHalfHeight - ActualHalfHeight) +
                                     TargetCharacterCDO->BaseEyeHeight;

      return TargetCharacter->GetActorLocation() +
             (GetNegativeGravityNormal() * HeightAdjustment);
    }
...
FVector AJoyPlayerCameraManager::GetCharacterFaceBase(const AActor *Target) {
  ...
  if (const APawn *TargetPawn = Cast<APawn>(TargetActor)) {
    if (const auto *SkeletalComp =
            TargetPawn->FindComponentByClass<USkeletalMeshComponent>()) {
      if (const USkeletalMesh *SkeletalMeshAsset =
...
FVector
AJoyPlayerCameraManager::GetCharacterCameraBase(const AActor *Target) const {
  if (Target == nullptr) {
    return FVector::ZeroVector;
  }
...
FRotator AJoyPlayerCameraManager::GetCurrentCameraArmCenterRotation(
    AActor *InViewTarget) const {
  if (InViewTarget != nullptr &&
      MultiViewTargetCameraManager.ContainsViewTarget(InViewTarget)) {
    return MultiViewTargetCameraManager[InViewTarget]
        .CurrentCamera.ArmCenterRotation;
  }
...
FViewTargetCameraInfo *
AJoyPlayerCameraManager::GetViewTargetCameraInfo(AActor *InViewTarget) {
  if (InViewTarget != nullptr &&
      MultiViewTargetCameraManager.ContainsViewTarget(InViewTarget)) {
    return &MultiViewTargetCameraManager[InViewTarget];
  }
...
FVector
AJoyPlayerCameraManager::GetCurrentArmCenterOffset(AActor *InViewTarget) const {
  if (InViewTarget != nullptr &&
      MultiViewTargetCameraManager.ContainsViewTarget(InViewTarget)) {
    return MultiViewTargetCameraManager[InViewTarget]
        .CurrentCamera.ArmCenterOffset;
  }
...
bool AJoyPlayerCameraManager::IsDuringMemberSwitching() const {
  if (const auto *PlayerController =
          UJoyGameBlueprintLibrary::GetJoyPlayerController(this)) {
...
void AJoyPlayerCameraManager::SetViewTarget(
    AActor *NewViewTarget, FViewTargetTransitionParams TransitionParams) {
  if (NewViewTarget) {
...
void AJoyPlayerCameraManager::SetViewTargetWithCurveBlend(
    AActor *NewViewTarget, TObjectPtr<UCurveFloat> BlendCurve,
    bool bEnableUpdateCameraConfig,
...
bool AJoyPlayerCameraManager::IsCurrentViewTarget(
    const AActor *TestViewTarget) const {
  return ViewTarget.Target.Get() == TestViewTarget;
}

bool AJoyPlayerCameraManager::IsPendingViewTarget(
    const AActor *TestViewTarget) const {
...
void AJoyPlayerCameraManager::ResetCameraToPlayer(float InBlendTime) const {
  if (const auto *CharacterControl =
          UJoyCharacterControlManageSubsystem::Get(GetWorld())) {
    AJoyCharacter *CurrentControlCharacter =
        CharacterControl->GetCurrentControlCharacter();
...
void AJoyPlayerCameraManager::BlendViewFunc_Default(FMinimalViewInfo &A,
                                                    FMinimalViewInfo &B,
                                                    float T) {
  A.BlendViewInfo(B, T);
}

void AJoyPlayerCameraManager::OnViewTargetChanged(APlayerController *PC,
...
void AJoyPlayerCameraManager::BlendViewFunc_ArcCurve(FMinimalViewInfo &A,
                                                     FMinimalViewInfo &B,
                                                     float T) {
  ...
  FVector MoveCurveCenter = (A.Location + B.Location) / 2.0;
  {
    ...
    float Angle = FMath::RadiansToDegrees(
        FMath::Acos(FVector::DotProduct(A_Direction, B_Direction)));
    if (Angle >= ThresholdWhenOffsetBegin) {
      ...
      // NormalDirection 方向
      if (FVector::DotProduct((A_Direction + B_Direction) * 0.5,
                              NormalDirection) > 0) {
      ...
      float XYOffsetScalar = FMath::Min(
          MaxXYOffsetParam_A * L + MaxXYOffsetParam_B, MaxXYOffsetParam_C);
  ...
  float MaxHeightDelta =
      FMath::Min(MaxHeightParam_A * L + MaxHeightParam_B, MaxHeightParam_C);
  MoveCurveCenter.Z += MaxHeightDelta;

  // 计算 Bezier 曲线上移动到的位置
  FVector Bezier_Loc = FMath::Square(1. - T) * A.Location +
                       2. * T * (1 - T) * MoveCurveCenter +
                       FMath::Square(T) * B.Location;
  ...
  {
    // Bezier 插值计算当前 pitch
    const float MaxPitchDegree =
        FMath::Max(-(PitchParam_A * MaxHeightDelta + PitchParam_B), -85);
    const float CurPitch = FMath::Square(1. - T) * A.Rotation.Pitch +
                           2. * T * (1 - T) * MaxPitchDegree +
  ...
  A.OrthoNearClipPlane =
      FMath::Lerp(A.OrthoNearClipPlane, B.OrthoNearClipPlane, T);
  A.OrthoFarClipPlane =
      FMath::Lerp(A.OrthoFarClipPlane, B.OrthoFarClipPlane, T);
  A.PerspectiveNearClipPlane =
      FMath::Lerp(A.PerspectiveNearClipPlane, B.PerspectiveNearClipPlane, T);
  A.OffCenterProjectionOffset =
      FMath::Lerp(A.OffCenterProjectionOffset, B.OffCenterProjectionOffset, T);
...
void AJoyPlayerCameraManager::BlendViewFunc_KeepViewDirection(
    FMinimalViewInfo &A, FMinimalViewInfo &B, float T) {
...
void AJoyPlayerCameraManager::SetBlendViewType(
    EJoyCameraBlendType InBlendViewWay) {
...
void AJoyPlayerCameraManager::RemoveBlendViewWay(
    EJoyCameraBlendType InBlendViewWay) {
...
void AJoyPlayerCameraManager::ProcessViewRotation(float DeltaTime,
  ...
  const auto *GravityManager = UJoyGravityManageSubsystem::Get(GetWorld());
  const FVector ViewPlaneZ = GravityManager != nullptr
                                 ? GravityManager->GetGravitySpaceZ()
                                 : FVector(0.f, 0.f, 1.0f);

  if (!OutDeltaRot.IsZero() && GravityManager) {
    const FRotator LocalViewRotation = UKismetMathLibrary::ComposeRotators(
        OutViewRotation, GravityManager->GetInverseGravitySpaceTransform());
    OutViewRotation = UKismetMathLibrary::ComposeRotators(
        LocalViewRotation + OutDeltaRot,
        GravityManager->GetGravitySpaceTransform());
  }
  ...
  if (OutViewRotation != OldViewRotation) {
    if (!ViewPlaneZ.IsZero()) {
      ...
      FRotationMatrix(OutViewRotation)
          .GetUnitAxes(ViewRotationX, ViewRotationY, ViewRotationZ);

      // Obtain angle (with sign) between current view Z vector and plane normal
      float PitchAngle =
          FMath::RadiansToDegrees(FMath::Acos(ViewRotationZ | ViewPlaneZ));
      ...
      if (PitchAngle > ViewPitchMax) {
        ...
        FQuat ViewRotation(
            FRotationMatrix::MakeFromZY(ViewPlaneZ, ViewRotationY));

        // Rotate 'up' with maximum pitch
        ViewRotation =
            FQuat(ViewRotationY, FMath::DegreesToRadians(-ViewPitchMax)) *
            ViewRotation;
        ...
        ViewRotation =
            FQuat(ViewRotationY, FMath::DegreesToRadians(-ViewPitchMin)) *
            ViewRotation;
...
AJoyPlayerCameraManager::GetCameraModifier(AActor *InActor) {
  return MultiViewTargetCameraManager.GetCameraModifier(InActor);
}

TObjectPtr<class UJoyCameraModifierController>
FMultiViewTargetCameraManager::GetCameraModifier(AActor *InViewTarget) {
...
bool FMultiViewTargetCameraManager::ContainsViewTarget(
    AActor *InViewTarget) const {
  return InViewTarget != nullptr &&
         ViewTargetCameraInfos.Contains(InViewTarget);
}

void FMultiViewTargetCameraManager::AddViewTarget(
    class AJoyPlayerCameraManager *CameraManager, AActor *InViewTarget,
    const FVirtualCamera &VirtualCamera) {
  if (InViewTarget == nullptr || ContainsViewTarget(InViewTarget) ||
  ...
  CameraInfo.CameraModifierController =
      NewObject<UJoyCameraModifierController>(CameraManager);
  check(CameraInfo.CameraModifierController);
