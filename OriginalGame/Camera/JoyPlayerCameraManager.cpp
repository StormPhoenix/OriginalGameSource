...
{
	/** ******************************************************************************** **
	 * (1) ViewTarget 处于 PlayerCameraManager 的控制中(ViewTarget、PendingViewTarget)
	 * (2) CameraModifierController 正在修改 ViewTarget
	 ** ******************************************************************************** **/
...
void AJoyPlayerCameraManager::InternalUpdateCamera(float DeltaTime)
{
	 ...
	 * 1. LastCamera: 存储 Fading、Modifiy 执行之前的相机状态，用于执行过程中的插值
	 * 2. DesiredCamera: 存储每帧计算出的期望相机状态
	 * 3. CurrentCamera: 存储每帧计算出的最终相机状态。它通常在 DesiredCamera 的基础上计算出来，
	 *		例如在 Fading 过程中，CurrentCamera 由 LastCamera 和 DesiredCamera 之间插值计算
	 *		得到。CurrentCamera 最终提供给外部，用于决定相机位置朝向。
	 */
	...
	/*
	 * 一部分相机参数修改后，会记录到标志位，于是在每帧修改之前，需把这些标志位置为 false
	 */
	...
	/*
	 * 上述代码修改过后的参数保留在 DesiredCamera 中，UpdateFading 将 DesiredCamera 数据同步到 CurrentCamera 中
	 */
	SyncDesiredCameraData(DeltaTimeThisFrame_IgnoreTimeDilation);
...
void AJoyPlayerCameraManager::SyncDesiredCameraData(float DeltaTime)
{
	for (TMap<TWeakObjectPtr<AActor>, FViewTargetCameraInfo>::TIterator ItViewTarget =
		     MultiViewTargetCameraManager.ViewTargetCameraInfos.CreateIterator();
	     ItViewTarget; ++ItViewTarget)
...
void AJoyPlayerCameraManager::UpdateActorTransform(float DeltaTime)
{
	     ...
	     ItViewTarget; ++ItViewTarget)
	{
		...
		FRotator LocalRotator = GravityManager != nullptr
...
void AJoyPlayerCameraManager::BlendViewFunc_ArcCurve(FMinimalViewInfo& A, FMinimalViewInfo& B, float T)
{
	...
	FVector MoveCurveCenter = (A.Location + B.Location) / 2.0;
	{
		...
		if (Angle >= ThresholdWhenOffsetBegin)
		{
			...
			// 判断向量 A B 方向合并向量和向量 NormalDirection 是否同向，如果否则反转 NormalDirection 方向
			if (FVector::DotProduct((A_Direction + B_Direction) * 0.5, NormalDirection) > 0)
			{
				NormalDirection = -NormalDirection;
			}

			// float XYOffsetScalar = FMath::Min(MaxXYOffsetParam_A * Angle + MaxXYOffsetParam_B, MaxXYOffsetParam_C);
...
bool AJoyPlayerCameraManager::BlockLookMoveInput_Implementation(UObject* InputReceiver,
