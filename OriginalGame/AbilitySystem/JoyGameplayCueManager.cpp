...
UJoyGameplayCueManager* UJoyGameplayCueManager::Get()
{
	return Cast<UJoyGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UJoyGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UJoyGameplayCueManager::LoadAlwaysLoadedCues()
{
...
bool UJoyGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
...
bool UJoyGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UJoyGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UJoyGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UJoyGameplayCueManager* GCM = Cast<UJoyGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogJoy, Error, TEXT("DumpGameplayCues failed. No UJoyGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogJoy, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogJoy, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogJoy, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogJoy, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogJoy, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogJoy, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	UE_LOG(LogJoy, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
...
void UJoyGameplayCueManager::HandlePostGarbageCollect()
{
...
void UJoyGameplayCueManager::ProcessLoadedTags()
{
...
void UJoyGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (JoyGameplayCueManagerCvars::LoadMode)
	{
	case EJoyEditorLoadMode::LoadUpfront:
...
void UJoyGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UJoyGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UJoyGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UJoyGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UJoyGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
