...
#include "QinGameplayCueManager.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(QinGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EQinEditorLoadMode
{
...
namespace QinGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("Qin.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via QinGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UQinGameplayCueManager::DumpGameplayCues));

	static EQinEditorLoadMode LoadMode = EQinEditorLoadMode::LoadUpfront;
}
...
UQinGameplayCueManager::UQinGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UQinGameplayCueManager* UQinGameplayCueManager::Get()
{
	return Cast<UQinGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UQinGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UQinGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogQin, Warning, TEXT("UQinGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UQinGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (QinGameplayCueManagerCvars::LoadMode)
	{
	case EQinEditorLoadMode::LoadUpfront:
		return true;
	case EQinEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EQinEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UQinGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UQinGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UQinGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UQinGameplayCueManager* GCM = Cast<UQinGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogQin, Error, TEXT("DumpGameplayCues failed. No UQinGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogQin, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogQin, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogQin, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogQin, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogQin, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogQin, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogQin, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogQin, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogQin, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogQin, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogQin, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogQin, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UQinGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UQinGameplayCueManager* StrongThis = Get())
					{
...
void UQinGameplayCueManager::HandlePostGarbageCollect()
{
...
void UQinGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogQin, Warning, TEXT("UQinGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UQinGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (QinGameplayCueManagerCvars::LoadMode)
	{
	case EQinEditorLoadMode::LoadUpfront:
...
void UQinGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UQinGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UQinGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UQinGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UQinGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UQinGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
